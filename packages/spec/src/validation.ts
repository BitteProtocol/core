...
import { fetchSpecWithRetry, formatOpenAPIErrors, formatZodError } from "./utils.js";

/**
 * Validation function for Bitte OpenAPI spec with "x-mb" extension.
 */
export async function validateBittePluginSpec(spec: string | object | URL | null | undefined): Promise<{
  valid: boolean;
  errors?: ErrorObject[];
  errorMessage?: string;
  ...
  try {
    ...
    const isUrl = spec instanceof URL || (typeof spec === "string" && spec.startsWith("http"));
    ...
    const { valid: openAPIValid, errors: openAPIErrors, schema: openAPISchema } = await validate(specContent, {
      throwOnError: false,
    });

    // Parse with zod for detailed validation and 'x-mb' extension validation
    const { success: schemaParseSuccess, error: schemaParseError, data: parsedSchema } = bitteOpenAPISpecZodSchema.safeParse(openAPISchema);

    if (!openAPIValid || !openAPISchema || !schemaParseSuccess || !parsedSchema) {
