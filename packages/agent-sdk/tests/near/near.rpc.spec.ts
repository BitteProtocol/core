...
describe("near utilities", () => {
  ...
  it("ftBalance success", async () => {
    expect(await ftBalance({accountId: "max-normal.near", contractId: "wrap.near"})).toBeDefined();
  });

  it("ftBalance zero", async () => {
    expect(await ftBalance({accountId: "zebra", contractId: "wrap.near"})).toBe("0");
  });

  it("ftBalance fail", async () => {
    await expect(() => ftBalance({accountId: "wrap.near", contractId: "zebra"})).toThrow();
  });
});
